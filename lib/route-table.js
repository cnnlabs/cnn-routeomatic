...
function RouteTable(src, config) {
    ...
    this.matchType = 'trie';
    this.resolver = this.handleTrieRoutes;
    this.routes = [];
    this.trie = null;
    ...
    routeLoop: for (let r, i = 0; i < src.routes.length; i++) {
        ...
        if (typeof r.hostMatch === 'string') {
            ...
            if (r.hostMatch.length !== 0 && !utils.isValidHostname(r.hostMatch)) {
        ...
        if (typeof r.protoMatch === 'string' && r.protoMatch.length > 0) {
            if (r.protoMatch.search(/^(http|https)$/) < 0) {
        ...
        if (typeof r.rewrite === 'string') {
            ...
            if (doSubs === true) {
                ...
                if (typeof r.match === 'string') {
                    r.match = utils.substitution(r.match, subs);
                }
            }
    ...
    if (typeof route.geoTarget === 'object' && route.geoTarget !== null) {
        ...
        for (let geo in route.geoTarget) {
            if (route.geoTarget.hasOwnProperty(geo)) {
                ...
                if ((newGeo.search(/^[A-Z]{2}$/) >= 0) || Array.isArray(continents[newGeo]) || Array.isArray(regions[newGeo])) {
...
RouteTable.prototype.prepRewriteRoute = function (route) {
    try {
        ...
        if (route.replace.search(/^(http|https)\:/i) >= 0 && route.redirectCode === 0) {
            route.redirectCode = this.defaultRedirectCode;
        }
        if (typeof route.status !== 'number') {
            route.status = 0;
        } else if (route.status !== 0 && (route.redirectCode < 400 || route.redirectCode > 505)) {
...
RouteTable.prototype.checkBasicsAndNormalizePath = function (req) {
    return (this.isCaseSensitive === true) ? req.path : req.normalizedPath;
};
...
RouteTable.prototype.handleMatchedRedirect = function (req, route, args) {
    ...
    if (route.geoTarget !== null) {
        ...
        req.log.debug('%s => (geotargeted) default is %s', args.key, route.redirect + qString, route.code);
        ...
        for (let geo in route.geoTarget) {
            if (route.geoTarget.hasOwnProperty(geo)) {
                if (geo.length === 2) {
                    ...
                    self.log.warn('Bad geotargeting value for "%s": %s', args.key, geo);
                }
            }
        }
        ...
        req.log.debug('%s => %s %d', args.key, route.redirect + qString, route.code);
...
RouteTable.prototype.handleMatchedRewrite = function (req, route, _args) {
    try {
        ...
        origParams = (qpos >= 0 ? req.url.substring(qpos + 1) : '');
        params = origParams;
        ...
        if (rule.status !== 0) {
            ...
            req.log.debug('rule #%d matched: %s => [%s]', matchOn, url, rule.status);
            req.send(rule.status, '');
            return true;  // Handled
        }

        if (rule.matchParams) {
            url = url.replace(rule.rewrite, rule.replace);
            qpos = url.indexOf('?');
            path = qpos >= 0 ? url.substr(0, qpos) : url;
            params = qpos >= 0 ? url.substr(qpos + 1) : '';
        } else {
            url = path.replace(rule.pattern, rule.replace);
            qpos = url.indexOf('?');
        ...
        if (rule.redirect !== 0) {
            req.log.debug('rule #%d matched: %s => %s [%s]', matchOn, req.url, url, rule.redirect);
            req.redirect(rule.redirect, url);
            return true;  // Handled
        } else {
            req.log.debug('rule #%d matched: %s => %s', matchOn, req.url, url);
        }
...
RouteTable.prototype.checkRegexRoutes = function (req) {
    ...
    key = self.checkBasicsAndNormalizePath(req);
    if (key === false) {
        return false;
    }

    for (let i = 0, rl = self.routes.length; i < rl; i++) {
        let r = self.routes[i],
            m = key.match(r.regex);
...
RouteTable.prototype.checkTrieRoutes = function (req) {
    ...
    if (result !== null) {
        ...
        if (typeof result.data.postMatchExp === 'object' && result.data.postMatchExp !== null &&
            result.data.postMatchExp instanceof RegExp && args[1].search(result.data.postMatchExp) === -1) {
