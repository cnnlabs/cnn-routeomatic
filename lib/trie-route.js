...
TrieRoute.prototype.add = function (path, data) {
    function inject(word, node, depth) {
        let first = word.charAt(depth),
            endIt = (word.length === depth || first === '#') ? true : false,
            val = (endIt === true ? '|' + (word.length === depth ? 'W' : 'X') : first);  // |W = substring match, |X = full string
