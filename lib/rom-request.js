...
RomRequest.prototype.proxy = function (options) {
    try {
        ...
        handleProxyResponse = function (opts, proxyResp) {
            ...
            if (!proxyResp || proxyResp.statusCode !== 200) {
                opts._rom.log.error(`Proxy request (${opts.fullUrl.href}) failed with status: ${(proxyResp && proxyResp.statusCode) || '???'}`);
                proxyResp.resume();
                opts._rom.error(502);
                return;
            }

            // proxyResp.setEncoding('utf8');
