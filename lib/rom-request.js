...
RomRequest.prototype.send = function (status, content) {
    ...
    try {
        ...
        if (this.type.length !== 0) {
            ...
            resp.type(Mime.lookup(this.path));
        }
...
RomRequest.prototype.proxy = function (options) {
    try {
        ...
        if (proxy.proto) {
            ...
            isHttp2 = (this.proto === 'https' && this.serverRequest.httpVersion.charAt(0) === '2');
            proto = this.proto + ':';
        }
        ...
        if (options.fullUrl.protocol === 'https:') {
            if (isHttp2 === true) {
                newReq = Http2.request(options.httpOpts, handleProxyResponse.bind(this, options));
            } else {
                newReq = Https.request(options.httpOpts, handleProxyResponse.bind(this, options));
            }
        } else {
            newReq = Http.request(options.httpOpts, handleProxyResponse.bind(this, options));
        }
...
RomRequest.prototype.rewrite = function (newUrl) {
    try {
        ...
        if (url.hostname !== null && ((url.hostname !== this.hostname) ||
            ...
            if (this.settings.removeDoubleSlashes === true && url.pathname.indexOf('//') >= 0) {
...
RomRequest.prototype.process = function (req, res, next) {
    ...
    try {
        ...
        this.proto = req.protocol;
        this.query = url.query;
        this.serverNext = next;
        this.serverResponse = res;
        this.serverRequest = req;
        this.serverPort = req.app.settings.port;
        this.url = req.url;
        ...
        if ((req.protocol === 'http' && this.port !== 80) || (req.protocol === 'https' && this.port !== 443)) {
            pstring = ':' + this.port.toString(10);
        }

        // Remove double slashes and redirect, if we should
        if (this.settings.removeDoubleSlashes === true && req.path.indexOf('//') >= 0) {
        ...
        if (this.settings.normalizeUrls === true) {
            ...
            if (normed.length !== req.url.length) {
                ...
                this.href = url.href;
                this.normalizedPath = url.pathname.toLowerString();
                this.path = url.pathname;
                this.query = url.query;
                this.url = req.url;
            }
