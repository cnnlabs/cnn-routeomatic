...
RomRequest.prototype.send = function (status, content) {
    ...
    try {
        ...
        if (this.type.length !== 0) {
            ...
            resp.type(Mime.getType(this.path));
        }
    ...
    try {
        ...
        handleProxyResponse = function (opts, proxyResp) {
            ...
            if (proxyRespCode < 200 || proxyRespCode >= 300) {
                ...
                if (options.httpOpts.proxyHeaders !== null) {
                    try {
                        utils.cloneResponseHeaders(servResp, options.httpOpts.proxyHeaders);
                    } catch (e) {
                        this.log.error(`Error attempting to set headers for proxied request: ${e.message}`);
                    }
                }
            }
