...
utils = {
    ...
    /**
     * Check request hostname, port, and protocol against route host, port, and protocol matching, if set
     *
     * @function
     * @public
     ...
     */
    doRuntimeChecks: function (req, route) {
        return ((route.methodMatch.length !== 0 && route.methodMatch !== req.method) ||
            (route.portMatch !== 0 && route.portMatch !== req.port) ||
            (route.hostMatch.length !== 0 && route.hostMatch !== req.hostname) ||
            (route.protoMatch.length !== 0 && route.protoMatch !== req.protocol)) ? false : true;
    },
     ...
     * @param {object} copyFrom - The response object to copy headers from
     */
    cloneResponseHeaders: function (copyTo, copyFrom) {
        for (let key in copyFrom.headers) {
            if (copyFrom.headers.hasOwnProperty(key)) {
                copyTo.setHeader(key, copyFrom.headers[key]);
            }
        }
    },
